...
def page_employees():
    ...
    with t1:
        ...
        st.markdown('<div class="sec-title">🗑️ Delete Employee</div>', unsafe_allow_html=True)
        del_emp_id = st.text_input('Enter Employee ID to delete')
        if st.button('Delete Employee', type='secondary'):
            if del_emp_id.strip() == ADMIN_EMP_ID:
                st.error('Cannot delete default admin user.')
            else:
                supabase.table('employees').delete().eq('emp_id', del_emp_id.strip()).execute()
                st.success(f'Employee {del_emp_id.strip()} deleted (if existed).')
