...
.main-header {
    ...
    color:white; padding:2rem 2.3rem; border-radius:18px;
    margin-bottom:1.4rem; box-shadow:0 10px 30px rgba(0,0,0,0.12);
}
...
.stat-card{flex:1;min-width:160px;background:white;border-radius:14px;
    padding:1.2rem;box-shadow:0 3px 14px rgba(0,0,0,0.07);border-top:4px solid;}
...
.stat-card .lbl{font-size:0.78rem;color:#777;margin-top:0.25rem;
    text-transform:uppercase;letter-spacing:0.5px;}

.file-card{background:white;border-radius:14px;padding:1rem 1.2rem;
...
.sec-title{font-size:1.05rem;font-weight:800;color:#1a1a2e;
    margin:1rem 0 0.6rem;padding-bottom:0.35rem;border-bottom:2px solid #e9ecef;}

.tbl-header{font-size:0.82rem;font-weight:700;color:#555;
    text-transform:uppercase;letter-spacing:0.4px;padding:0.4rem 0;
    border-bottom:2px solid #e9ecef;margin-bottom:0.3rem;}

.badge{display:inline-block;padding:0.12rem 0.55rem;border-radius:999px;
...
section[data-testid="stSidebar"] .stButton button:hover{
    background:rgba(233,69,96,0.35) !important;border-color:#e94560 !important;}
</style>
...
def file_ext_badge(name: str) -> str:
    ...
    cmap = {"py":"blue","zip":"orange","json":"purple","html":"orange","htm":"orange",
            "pdf":"red","csv":"green","xlsx":"green","xls":"green","txt":"green"}
...
def get_active_employees():
    r = supabase.table("employees").select(
        "emp_id,employee_name,email_id,is_admin,is_active"
    ).eq("is_active", True).order("employee_name").execute()
    return r.data or []

def get_all_employees():
    r = supabase.table("employees").select(
        "emp_id,employee_name,email_id,is_admin,is_active"
    ).order("employee_name").execute()
    return r.data or []
...
def ensure_admin():
    ...
    if not emp:
        supabase.table("employees").insert({
            "emp_id":ADMIN_EMP_ID,"employee_name":ADMIN_NAME,
            "email_id":ADMIN_EMAIL,"password_hash":hash_pw(ADMIN_EMP_ID),
...
def upload_to_supabase_storage(file_name: str, file_bytes: bytes, sender_emp_id: str):
    ...
    path       = f"{sender_emp_id}/{ts}_{safe_name}"
    ct         = mimetypes.guess_type(file_name)[0] or "application/octet-stream"
    supabase.storage.from_(DEFAULT_BUCKET).upload(
        path, file_bytes, file_options={"content-type": ct, "upsert": "false"}
    )
    return path
...
def create_shared_file(sender_emp_id, sender_name, filename,
                       storage_path, size_bytes, mime_type, file_hash, note, recipient_ids):
    res = supabase.table("shared_files").insert({
        "sender_emp_id":sender_emp_id,"sender_name":sender_name,
        "original_filename":filename,"storage_path":storage_path,
        "file_size_bytes":int(size_bytes),"mime_type":mime_type,
        "file_hash":file_hash,"note":note
    }).execute()
...
def log_activity(emp_id: str, action: str, details: str):
    try:
        supabase.table("file_activity_log").insert(
            {"emp_id":emp_id,"action_type":action,"details":details}
        ).execute()
    except:
        pass

def send_email_notification(to_emails: list, sender_name: str,
                            filenames: list, note: str = ""):
    ...
    try:
        ...
        html = f"""
        <div style="font-family:Inter,sans-serif;max-width:620px;margin:0 auto;
                    background:#fff;border-radius:16px;overflow:hidden;
                    box-shadow:0 4px 24px rgba(0,0,0,0.1)">
            <div style="background:linear-gradient(135deg,#1a1a2e,#0f3460,#e94560);
...
def login_screen():
    ...
    <div style="text-align:center;margin-top:3rem;margin-bottom:2rem">
        <div style="display:inline-block;
                    ...
                    box-shadow:0 8px 30px rgba(233,69,96,0.35)">📂</div>
        <h1 style="font-size:2rem;font-weight:800;color:#1a1a2e;
                   margin:0.9rem 0 0.2rem">{APP_NAME}</h1>
    ...
    with c2:
        with st.container(border=True):
            ...
            password = st.text_input("Password", type="password",
                                     placeholder="Default password = Employee ID")
...
def sidebar_nav():
    ...
    with st.sidebar:
        ...
        <div style="text-align:center;padding:1.1rem 0 1rem">
            <div style="background:linear-gradient(135deg,#e94560,#fb8500);
                        border-radius:50%;width:54px;height:54px;line-height:54px;
                        font-size:1.5rem;margin:0 auto 0.6rem;color:white">👤</div>
            <div style="font-size:0.95rem;font-weight:800;color:white">
                {emp["employee_name"]}</div>
        ...
        if st.button("🚪 Logout"):
            ...
            st.rerun()
        st.markdown(
            f"<p style='color:#94a3b8;font-size:0.72rem;text-align:center;margin-top:1rem'>"
            f"{APP_NAME}</p>",
            unsafe_allow_html=True
        )
...
def page_inbox():
    ...
    for f in items:
        ...
        with cols[0]:
            st.markdown(f"""
            <div class="file-card">
                <div class="fn">{file_ext_badge(f["original_filename"])} &nbsp;
                    {f["original_filename"]}</div>
                <div class="meta">📏 {human_size(f["file_size_bytes"])} &nbsp;|&nbsp;
                    🕐 {fmt_ts(f["created_at"])}</div>
                <div class="from">👤 Shared by: {f["sender_name"]}</div>
            </div>""", unsafe_allow_html=True)
        with cols[1]:
            try:
                b = download_from_storage(f["storage_path"])
                st.download_button("⬇️ Download", data=b,
                                   file_name=f["original_filename"],
                                   use_container_width=True,
                                   key=f"dl_{f['file_recipient_id']}")
            except:
                st.caption("⚠️ File missing")
        with cols[2]:
            if st.button("🗑️ Remove", use_container_width=True,
                         key=f"rm_{f['file_recipient_id']}"):
                supabase.table("file_recipients").update(
                    {"recipient_deleted": True}
                ).eq("id", f["file_recipient_id"]).execute()
                log_activity(emp_id,"recipient_delete",
                             f"Removed from inbox: {f['original_filename']}")
...
def page_send():
    ...
    st.markdown("""
    <div class="main-header"
         style="background:linear-gradient(135deg,#06d6a0,#118ab2,#073b4c)">
        <h1>📤 Send Files</h1>
    ...
    label_map  = {
        f"{e['employee_name']} (ID: {e['emp_id']})": e["emp_id"]
        for e in employees
    }
    c1,c2 = st.columns(2)
    with c1:
        st.markdown('<div class="sec-title">📎 Upload Files</div>',
                    unsafe_allow_html=True)
        uploads = st.file_uploader("Choose files", accept_multiple_files=True,
                                   label_visibility="collapsed")
        if uploads:
            st.caption(f"{len(uploads)} file(s) selected · "
                       f"Total: {human_size(sum(f.size for f in uploads))}")
    with c2:
        st.markdown('<div class="sec-title">👥 Select Recipients</div>',
                    unsafe_allow_html=True)
        selected = st.multiselect("Recipients", options=list(label_map.keys()),
                                  label_visibility="collapsed")

    note = st.text_area("Optional message",
                        placeholder="Add a note for recipients...", height=90)

    if st.button("🚀 Send Files Now", use_container_width=True, type="primary"):
        if not uploads:
            st.error("Please upload at least one file."); return
        if not selected:
            st.error("Please select at least one recipient."); return
        ...
        for i, uf in enumerate(uploads):
            ...
            ctype     = mimetypes.guess_type(uf.name)[0] or "application/octet-stream"
            spath     = upload_to_supabase_storage(uf.name, b, emp["emp_id"])
            create_shared_file(emp["emp_id"], emp["employee_name"],
                               uf.name, spath, len(b), ctype, fhash, note, recipient_ids)
            sent_names.append(uf.name)
            prog.progress((i+1)/len(uploads), text=f"Uploaded {i+1}/{len(uploads)}")

        ok, err = send_email_notification(
            recipient_emails, emp["employee_name"], sent_names, note)
        log_activity(emp["emp_id"],"send_files",
                     f"Sent {len(sent_names)} file(s) to {len(recipient_ids)} recipient(s)")
        st.success(f"✅ {len(sent_names)} file(s) shared with {len(recipient_ids)} recipient(s).")
        if ok:
...
def page_sent():
    ...
    st.markdown("""
    <div class="main-header"
         style="background:linear-gradient(135deg,#7209b7,#3a0ca3,#4361ee)">
        <h1>📋 Sent History</h1>
        <p>Files you have shared</p>
    </div>""", unsafe_allow_html=True)

    r     = supabase.table("shared_files").select("*").eq(
                "sender_emp_id", emp_id).order("created_at", desc=True).execute()
    items = r.data or []
    if not items:
        st.info("No sent files yet."); return

    for f in items:
        rc = supabase.table("file_recipients").select("id").eq(
                 "file_id", f["id"]).eq("recipient_deleted", False).execute()
        active_rec = rc.data or []
        ...
        with cols[0]:
            ...
            <div class="file-card">
                ...
                <div class="meta">📏 {human_size(f["file_size_bytes"])} &nbsp;|&nbsp;
                    🕐 {fmt_ts(f["created_at"])}</div>
                <div class="from">👥 Active recipients: {len(active_rec)}</div>
            </div>""", unsafe_allow_html=True)
        with cols[1]:
            if st.button("🗑️ Delete All", use_container_width=True,
                ...
                log_activity(emp_id,"sender_delete",
                             f"Deleted for all: {f['original_filename']}")
...
def page_employees():
    ...
    st.markdown("""
    <div class="main-header"
         style="background:linear-gradient(135deg,#fb8500,#e94560,#1a1a2e)">
    ...
    # ── Tab 1: Current Employees with row-wise Deactivate button ──────────────
    with t1:
        st.markdown('<div class="sec-title">All Employees</div>',
                    unsafe_allow_html=True)
        ...
        if not employees:
            ...
            # Header row
            h = st.columns([1.2,2.2,2.8,1.0,0.8,1.2])
            for col, lbl in zip(h,["Emp ID","Name","Email","Role","Active","Action"]):
                col.markdown(f"**{lbl}**")
            ...
            for emp in employees:
                c1,c2,c3,c4,c5,c6 = st.columns([1.2,2.2,2.8,1.0,0.8,1.2])
                role      = "Admin" if emp.get("is_admin") else "Employee"
                is_active = bool(emp.get("is_active"))

                c1.write(emp["emp_id"])
                c2.write(emp["employee_name"])
                c3.write(emp["email_id"])
                c4.write(role)
                c5.checkbox("", value=is_active,
                            key=f"chk_{emp['emp_id']}", disabled=True)

                with c6:
                    if emp["emp_id"] == ADMIN_EMP_ID:
                        st.write("—")
                    elif is_active:
                        if st.button("🗑 Deactivate",
                                     key=f"deact_{emp['emp_id']}",
                                     type="secondary",
                                     use_container_width=True):
                            ...
                            log_activity(
                                ...
                                "deactivate_employee",
                                f"Deactivated emp_id={emp['emp_id']} ({emp['employee_name']})"
                            )
                            st.success(
                                f"✅ {emp['employee_name']} ({emp['emp_id']}) deactivated."
                            )
                            st.rerun()
                    else:
                        st.markdown(
                            "<span style='color:#e94560;font-size:0.8rem;"
                            "font-weight:700'>Deactivated</span>",
                            unsafe_allow_html=True
                        )
    ...
    with t2:
        st.markdown('<div class="sec-title">Upload Employee List</div>',
                    unsafe_allow_html=True)
        st.markdown("**Required columns:** `emp_id` · `employee_name` · `email_id`  \n"
                    "Default password for each new employee = their Employee ID")
        up = st.file_uploader("Upload Excel or CSV", type=["xlsx","csv"], key="emp_up")
        if up is not None:
            try:
                df = pd.read_csv(up) if up.name.lower().endswith(".csv") else pd.read_excel(up)
                df.columns = [c.strip().lower().replace(" ","_") for c in df.columns]
                needed = {"emp_id","employee_name","email_id"}
                if not needed.issubset(set(df.columns)):
                    st.error(f"Missing columns. File must have: {needed}")
                else:
                    ...
                    if st.button("✅ Import Employees", type="primary"):
                        ...
                        for _, row in df.iterrows():
                            ...
                            if get_employee(eid):
                                ...
                                added += 1
                        st.success(f"✅ Imported {added} employee(s). Skipped {skipped} already existing.")
                        log_activity(ADMIN_EMP_ID,"import_employees",
    ...
    # ── Tab 3: Add Manually ───────────────────────────────────────────────────
    with t3:
        st.markdown('<div class="sec-title">Add Single Employee</div>',
                    unsafe_allow_html=True)
        with st.form("add_emp"):
            c1,c2 = st.columns(2)
            with c1:
                nid   = st.text_input("Employee ID *")
                nname = st.text_input("Employee Name *")
            with c2:
                nemail  = st.text_input("Email ID *")
                nadmin  = st.checkbox("Grant Admin Access")
            submitted = st.form_submit_button("➕ Add Employee",
                                              use_container_width=True)
        if submitted:
            nid = nid.strip()
            if not all([nid, nname.strip(), nemail.strip()]):
                st.error("All fields are required.")
            elif get_employee(nid):
                st.warning(f"Employee ID {nid} already exists.")
            else:
                supabase.table("employees").insert({
                    "emp_id":nid,"employee_name":nname.strip(),
                    "email_id":nemail.strip().lower(),
                    "password_hash":hash_pw(nid),
                    "is_admin":bool(nadmin),"is_active":True
                }).execute()
                st.success(f"✅ {nname} added. Default password: {nid}")
...
def page_admin_dashboard():
    ...
    total_size = sum(
        x.get("file_size_bytes",0)
        for x in (supabase.table("shared_files").select("file_size_bytes").execute().data or [])
    )

    st.markdown(f"""
    <div class="stat-row">
        <div class="stat-card blue">
            <div class="val">{emp_count}</div><div class="lbl">Employees</div></div>
        <div class="stat-card green">
            <div class="val">{file_count}</div><div class="lbl">Files Shared</div></div>
        <div class="stat-card orange">
            <div class="val">{rec_count}</div><div class="lbl">Deliveries</div></div>
        <div class="stat-card red">
            <div class="val">{human_size(total_size)}</div>
            <div class="lbl">Storage Used</div></div>
    </div>""", unsafe_allow_html=True)

    st.markdown('<div class="sec-title">Recent Activity Log</div>',
                unsafe_allow_html=True)
    r  = supabase.table("file_activity_log").select("*").order(
             "created_at", desc=True).limit(50).execute()
    df = pd.DataFrame(r.data or [])
    if not df.empty:
        df["created_at"] = df["created_at"].apply(fmt_ts)
        st.dataframe(df[["emp_id","action_type","details","created_at"]],
        ...
        with st.container(border=True):
            ...
            if st.button("🔒 Update Password",
                ...
                if emp["password_hash"] != hash_pw(cur):
                    ...
                    supabase.table("employees").update(
                        {"password_hash": hash_pw(new1)}
                    ).eq("emp_id", st.session_state.emp_id).execute()
...
def main():
    ...
    if   page == "inbox":     page_inbox()
    elif page == "send":      page_send()
    elif page == "sent":      page_sent()
    elif page == "employees": page_employees()
    elif page == "admin":     page_admin_dashboard()
    elif page == "password":  page_password()
    else:                     page_inbox()
